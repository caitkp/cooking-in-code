...
if ( ! isset( $content_width ) )
	$content_width = 523; /* pixels */

function fruit_shake_setup() {
	 ...
	 */
	load_theme_textdomain( 'fruit-shake', get_template_directory() . '/languages' );
...
add_action( 'after_setup_theme', 'fruit_shake_setup' );

/**
 * Enqueue scripts and styles.
 */
function fruit_shake_scripts() {
	wp_enqueue_style( 'fruit-shake', get_stylesheet_uri() );

	if ( is_singular() && comments_open() && get_option( 'thread_comments' ) )
		wp_enqueue_script( 'comment-reply' );
}
add_action( 'wp_enqueue_scripts', 'fruit_shake_scripts' );
...
/**
 * Check the current fruit scheme and return a default header image that
 * matches it.
 */
function fruit_shake_default_header_image( $url ) {
	$default_headers   = array(
		get_template_directory_uri() . '/images/headers/header-banana.png',
		get_template_directory_uri() . '/images/headers/header-blueberry.png',
		get_template_directory_uri() . '/images/headers/header-brown-banana.png',
		get_template_directory_uri() . '/images/headers/header-dragon-fruit.png',
	);

	if ( empty( $url ) || in_array( $url, $default_headers ) )
		$url = sprintf( '%1$s/images/headers/header-%2$s.png', get_template_directory_uri(), fruit_shake_current_fruit_scheme() );

	return $url;
}
add_filter( 'theme_mod_header_image', 'fruit_shake_default_header_image' );
